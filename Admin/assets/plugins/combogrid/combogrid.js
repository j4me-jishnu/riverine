	...
	_create: function() {
		...
		//ESCAPE needs this workaround
			...
			this.options.source =  function( request, response ) {
				self.createData(self,request);
				$.ajax({
					url: self.options.url,
					dataType: self.options.datatype,
					data:this.options.dataParam,
...
//		ul2.appendTo(ul);
    ...
    $("<div style='width:"+col.width+self.options.munit+";"+hide+" text-align:"+col.align+"' class='cg-DivItem "+col.addClass+"'>"+ colItem +"</div>").appendTo(div);
		});
		div.append("</div>");
...
// Select page
	...
	createData:function(self,request){
		var dataObj={
		...
		$.extend( this.options.dataParam, dataObj );
	}
});
